...
import pandas as pd
from laserGym.laser_cbc.envs.model import CBCModel
from feedback.recognizer import create_recognizer
...
class CombiningDataGen(Dataset):
    ...
    def __init__(self, **kwargs):
        ...
        self.double_frame = kwargs.get('double_frame', False) # False
        self.ortho_sampling = kwargs.get('ortho_sampling', False) #False
...
def train_model(model, dataset, device='cpu',
                patience=7, n_epochs=50, batch_size=64, verbose=False):
    model.to(device)
...
if __name__ == '__main__':
    ...
    p.add_argument('-m', type=int, default=3, choices=[3, 9], help='MxM beam shape') #9
    ...
    p.add_argument('--double_frame', action="store_true", help='doubled frame training') # "store_true"
    p.add_argument('--ortho_sampling', action="store_true", help='use random orthogonal sampling') # "store_true"
    p.add_argument("--rms_measure_noise", type=float, default=0.1, help="rms camera noise")
    p.add_argument("--phs_drift_step_deg", type=float, default=5, help="phs_drift_step_deg") #5
    p.add_argument("--dither_range_deg", type=int, default=45, help="dither phase range") #45
