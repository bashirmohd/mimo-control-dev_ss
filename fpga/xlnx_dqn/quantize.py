...
sys.path.append(os.path.join(os.path.dirname(__file__), '../..', 'feedback/physicsbased'))
# from recognizer import create_recognizer
import CNN

# from train_nn import CombiningDataGen, train_model
from trainCNN import CombiningDataGenCNN, train_CNNmodel
...
def quantize(dataset, model, weight, dim, double_frame, quant_mode, batchsize, quant_model, device):
    ...
    print(DIVIDER)
    print("")
    train_CNNmodel(quantized_model, dataset, device, n_epochs=43, verbose=False)
...
def run_main():
    ...
    ap.add_argument('-b', '--batchsize', type=int, default=80,
    ...
    net = CNN.Net(json_data['double_frame'], json_data['net_config'])
