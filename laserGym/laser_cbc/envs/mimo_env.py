...
class LaserControllerEnv(gym.Env, Controller):
    ...
    def __init__(self, **kwargs):
        ...
        self.action_space = gym.spaces.Box(
            low=-1, high=1, shape=(self.model.n_beams,),
            dtype=np.float32)

        self.observation_space = gym.spaces.Box(
            low=0, high=self.model.max_pwr, shape=(self.model.n_pattern_beams,),
            dtype=np.float32)
    ...
    def step(self, action):
        ...
        self.correct(action * np.pi)  # correct phases in 360 deg
        ...
        return self.model.nonzero_pattern, reward, done, info

    def reset(self):
        """ Perturb system state by injecting random phases """
        self.reset_drive()  # zero output
        self.model.reset()  # move to optimal state
        # perturb random phases in deg
        pattern, phs_perturb_deg = self.model.perturb_random(
            self.perturb_range, self.np_random)
        self.steps = 0
        return pattern
